...
const BALLOON_THEMES: CabinRaceHotAirBalloonTheme[] = ["blue", "green", "yellow", "purple", "white", "red"];

interface CabinRaceScoreTentProps {
  cabinInfo: CabinSummary[];
}

const CabinRaceScoreTent: React.FC<CabinRaceScoreTentProps> = ({ cabinInfo }) => {
  const maxPoints = 1000;
  const {isMobile, isTablet, isDesktop} = useDevice();

  return (
      <div className={`relative flex w-full ${ isMobile ? "h-[195vw]" : "h-[100vw]"} items-center justify-center`}>
        <CabinRaceTent
        ...
        <CabinRaceBush
            ...
            style={{transform: isDesktop ?  "translate(-33vw, 36vw)" :
                  isTablet ? "translate(-35vw, 40vw)" : "translate(-35vw, 80vw)"}}
        />
        <div className="relative flex flex-col gap-[1.5vw]">
          {cabinInfo.map((cabin, index) => (
            <div key={cabin.name} className={`flex items-start gap-[1vw]`}>
              ...
              <div className={`flex flex-col ${isMobile ? "w-[30vw]" : "w-[12vw]"}`}>
                <p className={`font-NeulisNeue-Bold text-white ${isMobile ? "text-[3vw]" : "text-[1.5vw]"}`}>{cabin.name}</p>
                <p className={`font-DMSans-Regular text-white ${isMobile ? "text-[2.5vw]" : "text-[1vw]"}`}>{cabin.points}</p>
              </div>
              <ProgressBar
